# Double DD Adventures

Enjoy an unforgettable adventure on the water with our best private charter boat service.

* Superior Boat Quality
* Afforable Prices
* Guaranteed Security

## Booking Details

WhatsApp Booking: https://wa.me/2976613950
Instagram Booking: https://www.instagram.com/double_dd_adventures/

## Media

The photos and videos are stored in the media folder with each media having a descriptive name. The reviews are prefixed with "Screenshot"

## Website requirements 

* The primary goal of the website is to potential customer to book and/or inquire for services using either WhatsApp or Instagram.
* The website shall be optimized for mobile first, but also allow a presentable view for tablet/desktop viewports. 
* The website shall be a two page site where the first page shall quickly allow the user to scroll and view it's services.
* The website shall have a single page dedicated to showing the screenshot of customer reviews.
* The media shall be group by activity type of the following:
  * Sunsets
  * Family
  * Snorkeling
  * Fishing
  * Boat
  * The captain crew
  * Instagram baddies
  * Reviews
  * Parties
  * Intimate couple dates
* The website shall be build so that it does not need to be compiled and can run as is locally and on other hosting platforms.
* Inbetween each category, the website shall display a single review screenshot.

## Running the site

The site is fully static—no build steps or package installs required.

* Open `index.html` for the experience/services page with all media grouped by category and review screenshots interleaved between each section.
* Open `reviews.html` for the full wall of customer-review screenshots.
* All assets live in `photos/`, so you can serve locally with any static server (or double-click `index.html`).
* Booking CTAs for WhatsApp and Instagram are surfaced in the header, hero, and review sections on both pages for quick conversion without clutter.



